...
<head>
  <meta charset="UTF-8">
  <title>New Order Notification - {{ config('app.name') }}</title>
  <style>
    body {
...
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <p>A new order has just been placed by <strong>{{ $customer->name }}</strong> on your store.</p>

      <div class="details">
        <p><strong>Order Number:</strong> {{ $order->order_number }}</p>
        <p><strong>Total Amount:</strong> {{ number_format($order->total, 2) }} {{ $order->currency }}</p>
        <p><strong>Payment Method:</strong> {{ $order->payment_method }}</p>
        <p><strong>Order Date:</strong> {{ $order->created_at->format('F j, Y - H:i A') }}</p>
      </div>
