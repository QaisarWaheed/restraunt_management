  ...
  <div class="Categories-page">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        <button class="btn btn-outline-secondary me-2" @click="showDeletedModal">
          <i class="fas fa-trash-restore-alt me-1"></i> {{ $t('tenant_dashboard.categories.deletedCategories') }}
        </button>
      </div>
      <h4 class="mb-0">{{ $t('tenant_dashboard.categories.title') }}</h4>
      <button class="btn btn-add-Category" @click="showCreateModal">
        <i class="fas fa-plus me-2"></i>{{ $t('tenant_dashboard.categories.addCategory') }}
      </button>
    </div>
      ...
      <div class="filter-body">
        <div class="row g-3 align-items-end flex-nowrap flex-md-nowrap flex-column flex-md-row">
          <div class="col-md-4 col-12 mb-2 mb-md-0">
            <label class="form-label mb-1">{{ $t('tenant_dashboard.categories.search') }}</label>
            <input v-model="filters.search" @input="applyFilters" type="text" class="form-control filter-input" :placeholder="$t('tenant_dashboard.categories.searchPlaceholder')">
          </div>
          <!-- Responsive Category Dropdown: only show on small screens -->
          <div class="col-12 d-md-none mb-2">
            <label class="form-label mb-1">{{ $t('tenant_dashboard.categories.tableHeaders.name') }}</label>
            <select v-model="filters.parent_id" @change="applyFilters" class="form-select filter-input">
              <option value="">{{ $t('tenant_dashboard.categories.allCategories') }}</option>
              <option v-for="cat in parentCategories" :key="cat.id" :value="cat.id">{{ cat.name }}</option>
            </select>
          </div>
          <!-- Desktop: keep as is, but hidden on small screens -->
          ...
          <div class="col-md-4 col-12 mb-2 mb-md-0">
            <label class="form-label mb-1">{{ $t('tenant_dashboard.categories.tableHeaders.status') }}</label>
            <select v-model="filters.is_active" @change="applyFilters" class="form-select filter-input">
              <option value="">{{ $t('tenant_dashboard.categories.allStatus') }}</option>
              <option :value="true">{{ $t('tenant_dashboard.categories.statusBadges.active') }}</option>
              <option :value="false">{{ $t('tenant_dashboard.categories.statusBadges.inactive') }}</option>
            </select>
          </div>
        </div>
      </div>
    ...
    <div class="card">
      <div class="card-body">
        ...
        <div v-else-if="categories.length === 0" class="text-center py-5 text-muted">
          ...
          <div class="h5 mb-2">{{ $t('tenant_dashboard.categories.noCategoriesFound') }}</div>
          <div>{{ $t('tenant_dashboard.categories.noCategoriesMessage') }}</div>
        </div>
          ...
          <table class="table table-hover table-sm Categories-table">
            <thead>
              <tr>
                ...
                <th>{{ $t('tenant_dashboard.categories.tableHeaders.name') }}</th>
                <!-- <th class="d-none d-md-table-cell">Parent</th> -->
                <th class="d-none d-md-table-cell">{{ $t('tenant_dashboard.categories.tableHeaders.status') }}</th>
                <th class="d-none d-md-table-cell">{{ $t('tenant_dashboard.categories.tableHeaders.sortOrder') }}</th>
                <th class="d-none d-md-table-cell">{{ $t('tenant_dashboard.categories.tableHeaders.createdAt') }}</th>
                <th class="d-none d-md-table-cell">{{ $t('tenant_dashboard.categories.tableHeaders.actions') }}</th>
              </tr>
            </thead>
                ...
                <tr :key="category.id" :class="['d-block d-md-table-row w-100 mb-2 mb-md-0', { 'table-danger': category.deleted_at }]">
                  ...
                  <td class="d-none d-md-table-cell">
                    <span class="badge" :class="category.is_active ? 'bg-success' : 'bg-danger'">
                      {{ category.is_active ? $t('tenant_dashboard.categories.statusBadges.active') : $t('tenant_dashboard.categories.statusBadges.inactive') }}
                    </span>
                  </td>
        ...
        <div v-if="pagination.total > 0" class="d-flex justify-content-between align-items-center mt-3">
          <div class="text-muted">
            {{ $t('tenant_dashboard.categories.showingEntries', { from: pagination.from, to: pagination.to, total: pagination.total }) }}
          </div>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content Category-modal-content">
          <div class="modal-header Category-modal-header">
            <h5 class="modal-title Category-modal-title">{{ isEditing ? $t('tenant_dashboard.categories.editCategory') : $t('tenant_dashboard.categories.addCategory') }}</h5>
              ...
              <div class="row g-3 mb-3">
                <div class="col-md-6">
                  <div class="input-group">
                    ...
                    <input id="categoryName" type="text" class="form-control" v-model="form.name" required :placeholder="$t('tenant_dashboard.categories.form.namePlaceholder')">
                  </div>
                </div>
                <!-- <div class="col-md-6">
                  <div class="input-group">
                ...
                <div class="col-md-6">
                  <div class="form-check form-switch mt-2">
                    ...
                    <label class="form-check-label" for="categoryStatus">{{ $t('tenant_dashboard.categories.form.activeLabel') }}</label>
                  </div>
                </div>
                <div class="col-12">
                  <div class="input-group">
                    <span class="input-group-text"><i class="fas fa-align-left"></i></span>
                    <textarea id="categoryDescription" class="form-control" v-model="form.description" rows="3" :placeholder="$t('tenant_dashboard.categories.form.descriptionPlaceholder')"></textarea>
                  </div>
                </div>
                <!-- <div class="col-12">
                  <div class="input-group">
                    <span class="input-group-text"><i class="fas fa-image"></i></span>
                    <input type="text" class="form-control" v-model="form.image" :placeholder="$t('tenant_dashboard.categories.form.imagePlaceholder')">
                  </div>
                </div> -->
              </div>
              <div class="modal-footer Category-modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ $t('tenant_dashboard.categories.modal.cancel') }}</button>
                <button type="submit" class="btn btn-submit">{{ isEditing ? $t('tenant_dashboard.categories.modal.update') : $t('tenant_dashboard.categories.modal.create') }}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ $t('tenant_dashboard.categories.viewCategoryDetails') }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body">
            <div class="Category-details">
              <p><strong>{{ $t('tenant_dashboard.categories.view.name') }}:</strong> {{ selectedCategory?.name }}</p>
              <p><strong>{{ $t('tenant_dashboard.categories.view.description') }}:</strong> {{ selectedCategory?.description }}</p>
              <p><strong>{{ $t('tenant_dashboard.categories.view.parent') }}:</strong> {{ selectedCategory?.parent?.name || $t('tenant_dashboard.categories.view.none') }}</p>
              <p><strong>{{ $t('tenant_dashboard.categories.view.status') }}:</strong> {{ selectedCategory?.is_active ? $t('tenant_dashboard.categories.statusBadges.active') : $t('tenant_dashboard.categories.statusBadges.inactive') }}</p>
              <p><strong>{{ $t('tenant_dashboard.categories.view.sortOrder') }}:</strong> {{ selectedCategory?.sort_order }}</p>
              <p v-if="selectedCategory?.image"><strong>{{ $t('tenant_dashboard.categories.view.image') }}:</strong> <img :src="selectedCategory.image" class="img-thumbnail mt-2" style="max-width: 200px;"></p>
              <p><strong>{{ $t('tenant_dashboard.categories.view.createdAt') }}:</strong> {{ new Date(selectedCategory?.created_at).toLocaleString() }}</p>
              <p v-if="selectedCategory?.deleted_at">
                <strong>{{ $t('tenant_dashboard.categories.view.deletedAt') }}:</strong> {{ new Date(selectedCategory?.deleted_at).toLocaleString() }}
              </p>
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ $t('tenant_dashboard.categories.modal.close') }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal fade" id="deletedCategoriesModal" tabindex="-1">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ $t('tenant_dashboard.categories.deletedCategoriesTitle') }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body">
            <div v-if="deletedCategories.length === 0" class="text-center py-4 text-muted">
              <i class="fas fa-store-slash fa-2x mb-2"></i>
              <div>{{ $t('tenant_dashboard.categories.noDeletedCategoriesFound') }}</div>
            </div>
            <div v-else class="table-responsive">
              <table class="table table-hover table-sm">
                <thead>
                  <tr>
                    <th>{{ $t('tenant_dashboard.categories.tableHeaders.name') }}</th>
                    <th>{{ $t('tenant_dashboard.categories.tableHeaders.description') }}</th>
                    <th>{{ $t('tenant_dashboard.categories.tableHeaders.deletedAt') }}</th>
                    <th>{{ $t('tenant_dashboard.categories.tableHeaders.action') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="cat in deletedCategories" :key="cat.id">
                    ...
                    <td>
                      <button class="btn btn-sm btn-success" @click="restoreCategory(cat)"><i class="fas fa-undo"></i> {{ $t('tenant_dashboard.categories.restore') }}</button>
                    </td>
                  </tr>
                </tbody>
              </table>
  ...
  setup() {
    ...
    const fetchCategories = async (page = 1) => {
      ...
      try {
        ...
        categories.value = []
        pagination.value = { current_page: 1, last_page: 1, per_page: 5, total: 0, from: 0, to: 0 }
      } finally {
        loading.value = false
      }
    }
    ...
    const saveCategory = async () => {
      try {
        ...
        Swal.fire({
          icon: 'success',
          title: isEditing.value ? $t('tenant_dashboard.categories.categoryUpdated') : $t('tenant_dashboard.categories.categoryCreated'),
          toast: true,
          position: 'top-end',
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: error.response?.data?.message || $t('tenant_dashboard.categories.messages.saveFailed')
        })
      }
    }
    ...
    const confirmDelete = (category) => {
      Swal.fire({
        ...
        text: $t('tenant_dashboard.categories.messages.confirmDelete'),
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          try {
            ...
            Swal.fire({
              icon: 'success',
              title: $t('tenant_dashboard.categories.messages.deleteSuccess'),
              toast: true,
              position: 'top-end',
              showConfirmButton: false,
              timer: 3000
            ...
            Swal.fire({
              icon: 'error',
              title: 'Error',
              text: error.response?.data?.message || $t('tenant_dashboard.categories.messages.deleteFailed')
            })
          }
        }
      })
    ...
    const confirmHardDelete = (category) => {
      Swal.fire({
        title: $t('dashboard_common.confirm'),
        text: $t('tenant_dashboard.categories.messages.confirmHardDelete'),
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
        cancelButtonColor: '#3085d6',
        ...
        if (result.isConfirmed) {
          try {
            ...
            Swal.fire({
              icon: 'success',
              title: $t('tenant_dashboard.categories.messages.hardDeleteSuccess'),
              toast: true,
              position: 'top-end',
              showConfirmButton: false,
              timer: 3000
            ...
            Swal.fire({
              icon: 'error',
              title: 'Error',
              text: error.response?.data?.message || $t('tenant_dashboard.categories.messages.hardDeleteFailed')
            })
          }
        }
      })
    ...
    const restoreCategory = async (category) => {
      try {
        ...
        Swal.fire({
          icon: 'success',
          title: $t('tenant_dashboard.categories.messages.restoreSuccess'),
          toast: true,
          position: 'top-end',
          showConfirmButton: false,
          timer: 3000
        ...
        console.error('Error restoring category:', error)
            ...
            icon: 'error',
            title: 'Error',
            text: error.response?.data?.message || $t('tenant_dashboard.categories.messages.restoreFailed')
          })
      }
    }
    ...
    const onSortEnd = async () => {
      try {
        ...
        await fetchCategories();
        Swal.fire({ icon: 'success', title: $t('tenant_dashboard.categories.orderUpdated'), toast: true, position: 'top-end', showConfirmButton: false, timer: 1500 });
      } catch (error) {
        Swal.fire({ icon: 'error', title: 'Error', text: $t('tenant_dashboard.categories.orderUpdateFailed') });
      }
    };
