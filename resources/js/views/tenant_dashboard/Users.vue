          ...
          <table class="table table-hover table-sm users-table">
            <thead>
              <tr>
                ...
                <th class="d-none d-md-table-cell">{{ $t('tenant_dashboard.users.tableHeaders.role') }}</th>
                <th class="d-none d-md-table-cell">{{ $t('tenant_dashboard.users.tableHeaders.status') }}</th>
                <th class="d-none d-md-table-cell">{{ $t('tenant_dashboard.users.tableHeaders.createdAt') }}</th>
                <th class="d-none d-md-table-cell">{{ $t('tenant_dashboard.users.tableHeaders.actions') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="user in users" :key="user.id" :class="['d-block d-md-table-row w-100 mb-2 mb-md-0', { 'table-danger': user.deleted_at }]">
                <td>
                  ...
                  <div class="d-md-none small mt-1">
                    ...
                    <div>{{ $t('tenant_dashboard.users.created') }}: {{ new Date(user.created_at).toLocaleDateString() }}</div>
                    <div class="mt-1">
                      <div class="btn-group">
              ...
              <div class="row g-3 mb-3">
                <div class="col-md-6">
                  <div class="input-group">
                    ...
                    <input id="userName" type="text" class="form-control" v-model="form.name" required placeholder="Full Name" data-bs-toggle="tooltip" title="Enter the user's full name">
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="input-group">
                    <span class="input-group-text"><i class="fas fa-envelope"></i></span>
                    <input id="userEmail" type="email" class="form-control" v-model="form.email" required placeholder="user@email.com" data-bs-toggle="tooltip" title="Enter a valid email address">
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="input-group">
                ...
                <div class="col-md-6">
                  <div class="input-group">
                    ...
                    <select id="userRole" class="form-select" v-model="form.role" required data-bs-toggle="tooltip" title="Select a role for this user">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="user-details">
              ...
              <p><strong>{{ $t('tenant_dashboard.users.createdAt') }}:</strong> {{ new Date(selectedUser?.created_at).toLocaleString() }}</p>
              <p v-if="selectedUser?.deleted_at">
                <strong>{{ $t('tenant_dashboard.users.deletedAt') }}:</strong> {{ new Date(selectedUser?.deleted_at).toLocaleString() }}
              </p>
            </div>
          </div>
...
export default {
  ...
  setup() {
    ...
    const saveUser = async () => {
      try {
        if (isEditing.value) {
          await axios.put(`/tenant/users/${form.value.id}`, form.value)
        } else {
          await axios.post('/tenant/users', form.value)
        }
