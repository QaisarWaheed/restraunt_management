...
const selectedPage = ref(null)
    ...
    const showDeletedModal = async () => {
      try {
        ...
        Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to fetch deleted pages' })
      }
    }
  ...
  selectedPage.value = page
    ...
    const savePage = async () => {
      try {
        ...
        Swal.fire({
          icon: 'success',
          title: isEditing.value ? 'Page Updated' : 'Page Created',
          toast: true,
          position: 'top-end',
    ...
    const confirmHardDelete = (page) => {
      Swal.fire({
        ...
        text: "This will permanently delete the page. This action cannot be undone!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
        cancelButtonColor: '#3085d6',
        ...
        if (result.isConfirmed) {
          try {
            ...
            Swal.fire({
              icon: 'error',
              title: 'Error',
              text: error.response?.data?.message || 'Failed to permanently delete page'
            })
          }
        }
      })
    ...
    const restorePage = async (page) => {
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: error.response?.data?.message || 'Failed to restore page'
        })
      }
    }
