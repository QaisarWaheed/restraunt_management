            ...
            <draggable v-model="products" @end="onSortEnd" tag="tbody" :animation="200" handle=".drag-handle">
              <template #item="{ element: product }">
                <tr :key="product.id" :class="['d-block d-md-table-row w-100 mb-2 mb-md-0', { 'table-danger': product.deleted_at }]">
                  ...
                  <td class="d-none d-md-table-cell">{{ product.category?.name || 'None' }}</td>
                  <td class="d-none d-md-table-cell">{{ product.price }}</td>
                  <td class="d-none d-md-table-cell">{{ product.cost_price }}</td>
                  <td class="d-none d-md-table-cell">{{ product.stock_quantity }}</td>
                  <td class="d-none d-md-table-cell">{{ product.min_stock_level }}</td>
                  ...
                  <td class="d-none d-md-table-cell">{{ product.sort_order }}</td>
                  <td class="d-none d-md-table-cell">{{ new Date(product.created_at).toLocaleDateString() }}</td>
                  <td class="d-none d-md-table-cell">
                    <div class="btn-group">
...
export default {
  ...
  setup() {
    ...
    const selectedColumns = ref(['name', 'category', 'price', 'stock_quantity'])
    ...
    const fetchProducts = async (page = 1) => {
      ...
      try {
        ...
        const { data } = await axios.get('/tenant/products', { params })
        const d = data.data
