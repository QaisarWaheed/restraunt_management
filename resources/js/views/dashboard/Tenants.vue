          ...
          <div class="col-md-4">
            <div class="input-group">
              ...
              <button class="btn btn-outline-secondary">
                <i class="fas fa-search" :title="$t('common.search')"></i>
              </button>
            </div>
          </div>
          <div class="col-md-3">
            <select class="form-select" v-model="statusFilter">
              ...
              <option value="inactive">{{ $t('tenants.status.inactive') || 'Inactive' }}</option>
          ...
          <table class="table table-hover table-sm align-middle">
            <thead>
              <tr>
                ...
                <th class="d-none d-md-table-cell">{{ $t('tenants.list.owner') }}</th>
                <th class="d-none d-md-table-cell">{{ $t('tenants.list.domain') }}</th>
                <th class="d-none d-md-table-cell">{{ $t('tenants.list.status') }}</th>
                <th class="d-none d-md-table-cell">{{ $t('common.actions') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="tenant in filteredTenants" :key="tenant.id" class="d-block d-md-table-row w-100 mb-2 mb-md-0">
                <td>
                  <div class="d-flex align-items-center">
                    <img
                      :src="tenant.logo || '/images/default-restaurant.png'"
                      :alt="tenant.name"
                      class="tenant-logo me-2"
                    >
                    <div>
                      <div class="fw-bold">{{ tenant.name }}</div>
                      <small class="text-muted">Created: {{ formatDate(tenant.created_at) }}</small>
                      <div class="d-md-none small mt-1">
                        <div>{{ $t('tenants.list.owner') }}: {{ tenant.owner_name }} ({{ tenant.owner_email }})</div>
                        <div>{{ $t('tenants.list.domain') }}: {{ tenant.subdomain }}.yourdomain.com</div>
                        <div>{{ $t('tenants.list.status') }}: <span :class="['badge', getStatusBadgeClass(tenant.status)]">{{ tenant.status }}</span></div>
                        <div class="mt-1">
                          <select
                ...
                <td class="d-none d-md-table-cell">{{ tenant.subdomain }}.yourdomain.com</td>
                <td class="d-none d-md-table-cell">
...
export default {
  ...
  setup() {
      ...
      return tenants.value
        .filter(tenant => {
          const matchesSearch = !searchQuery.value ||
            ...
            tenant.owner_name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
            tenant.owner_email.toLowerCase().includes(searchQuery.value.toLowerCase())
      ...
      try {
        ...
        if (error.response?.status !== 404) {
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: error.response?.data?.message || 'Failed to load restaurants. Please try again.'
          })
        } else {
          tenants.value = []
        }
    ...
    const approveTenant = async (tenant) => {
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: error.response?.data?.message || 'Failed to approve restaurant'
        })
      } finally {
        loading.value = false
      }
    ...
    const suspendTenant = async (tenant) => {
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: error.response?.data?.message || 'Failed to suspend restaurant'
        })
      } finally {
        loading.value = false
      }
    ...
    const activateTenant = async (tenant) => {
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: error.response?.data?.message || 'Failed to activate restaurant'
        })
      } finally {
        loading.value = false
      }
    ...
    const changeTenantStatus = async (tenant, status) => {
      try {
        loading.value = true
        await axios.patch(`/dashboard/tenants/${tenant.id}/status`, { status })
        await fetchTenants()
        Swal.fire({
          icon: 'success',
          title: 'Success',
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: error.response?.data?.message || 'Failed to change tenant status'
        })
      } finally {
        loading.value = false
      }
