      ...
      <div class="col-md-6">
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="card-title mb-0">{{ $t('dashboard.recentTenants') }}</h5>
            <router-link to="/dashboard/tenants" class="btn btn-sm btn-primary">
              {{ $t('common.viewAll') }}
            </router-link>
          </div>
          <div class="card-body">
      ...
      <div class="col-md-6">
        <div class="card">
          <div class="card-header">
            <h5 class="card-title mb-0">{{ $t('dashboard.systemStats') }}</h5>
          </div>
          <div class="card-body">
            <div class="list-group">
              ...
              <div class="list-group-item d-flex justify-content-between align-items-center">
                ...
                <span class="badge bg-primary rounded-pill">{{ stats.totalRoles }}</span>
              </div>

              <div class="list-group-item d-flex justify-content-between align-items-center">
                {{ $t('main_dashboard.stats.activeUsers') }}
                <span class="badge bg-success rounded-pill">{{ stats.activeUsersToday }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card mt-4">
          <div class="card-header">
            <h5 class="card-title mb-0">{{ $t('main_dashboard.quickActions.title') }}</h5>
          </div>
          <div class="card-body">
            <div class="row g-3">
              <div class="col-6">
                <router-link to="/dashboard/tenants" class="quick-action-card">
                  <i class="fas fa-store"></i>
                  <span>{{ $t('main_dashboard.quickActions.manageTenants') }}</span>
                </router-link>
              </div>
              <div class="col-6">
                <router-link to="/dashboard/users" class="quick-action-card">
                  <i class="fas fa-users"></i>
                  <span>{{ $t('main_dashboard.quickActions.manageUsers') }}</span>
                </router-link>
              </div>
              <div class="col-6">
                <router-link to="/dashboard/roles" class="quick-action-card">
                  <i class="fas fa-user-shield"></i>
                  <span>{{ $t('main_dashboard.quickActions.manageRoles') }}</span>
                </router-link>
              </div>
              <div class="col-6">
                <a href="#" class="quick-action-card" @click.prevent="clearCache">
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (tenantsRes.data && tenantsRes.data.data) {
          ...
          recentTenants.value = tenantsRes.data.data.filter(t => t.created_at && t.created_at.slice(0, 10) === today)
        }
      } catch (err) {
